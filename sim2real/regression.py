...
val_out_df = pd.DataFrame(columns = ['index','s1','flh','flk','frh','frk','s2','blh','blk','brh','brk'])
...
out_df = pd.read_csv("action1_out.csv")
...
np.save('A_matrix.npy', reg.coef_)
np.save('B_matrix.npy', reg.intercept_)
