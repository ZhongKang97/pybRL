...
class Stoch2Env(gym.Env):
    ...
    def __init__(self,
        ...
        self._theta = 0
        self._theta0 = 0
        self._update_action_every = 5.  # update is every 50% of the step i.e., theta goes from 0 to pi/2
        self._frequency = 1.
        self._kp = 20
        self._kd = 2.0
        self.dt = 0.001
...
#             print(theta,qpos_act[[1,2,3,4,6,7,8,9]])
    ...
    def _get_reward(self,action,energy_spent_per_step,cost_reference):
       ...
       # print(rpy)
        ...
        # distance_travelled = ((current_base_position[0] - self._last_base_position[0])**2 + (current_base_position[2] - self._last_base_position[2])**2)**0.5 +  # added negative reward for staying still
        distance_travelled = current_base_position[0] - self._last_base_position[0]
...
#         stride_length_reward = 0.5 * np.exp(-10*(1.0 - (rt_start[1] - rt_start[3]))**2) + 0.1 * np.exp(-10*(0.0 - (rt_mid[1] - rt_mid[3]))**2)
        ...
        reward = distance_travelled - penalty - 0.01 * energy_spent_per_step + 0.5 * costreference_reward + 0.1*walking_height_reward #+ foot_clearance_reward + stride_length_reward# + walking_velocity_reward
