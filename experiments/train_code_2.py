...
agent = npg.NPG(env, policy, baseline, normalized_step_size=5e-3, seed=None, hvp_sample_frac=1.0, save_logs=True, kl_dist=None, FIM_invert_args={'iters': 50, 'damping': 1e-4},)
# agent = ppo.PPO(env, policy, baseline, clip_coef=0.2, epochs=10, mb_size=64, learn_rate=3e-4, save_logs=True)
args = dict(
                job_name = 'MinitaurTrot_exp2',
                agent = agent,
                seed = 0,
                niter = 5000,
                gamma = 0.995,
