...
RENDER_HEIGHT = 720 #1080 #360
RENDER_WIDTH = 960 #1960 #480
...
class MinitaurBulletEnv(gym.Env):
  ...
  def _reset(self):
    if self._hard_reset:
      ...
      plane = self._pybullet_client.loadURDF("/home/abhik/stairs/urdf/stairs.urdf")
  ...
  def _step(self, action):
    ...
    #   action_final[6:] = action_half[:2]

    # if self.canter:
    #   action_final[:2] = action_half[:2]
    #   action_final[2:4] = action_half[:2]
    ...
    action_final[8] =   5 * math.pi/180 * (action[4])
    action_final[9] = -5 * math.pi/180 * (action[4])

    # action_final[8] = action[-1]
    # action_final[9] = -action[-1]

    # print(action, "###", action_half, "$$$$", action_final)



    for _ in range(self._action_repeat):
      self.minitaur.ApplyAction(action_final)
      self._pybullet_client.stepSimulation()
  ...
  def is_fallen(self):
    ...
    # vel = self.minitaur.GetBaseVelocity()
    # print(type(vel), type(pos))

    if self.log_buff:
      obs = list(pos)
      # obs.extend(list(pos))
      obs.extend(list(self.minitaur.GetBaseVelocity()))
      self.cg_list.append(obs)
      self.c+=1
      print("cg ",self.c)
      if self.c==900:
        np.savetxt("/home/abhik/batch-ppo/logdir/bound/cg.txt", self.cg_list)
        print("data saved")
  ...
  def gauss(self, x, mean, sd):
    ...
    gauss = math.e**(-(float(x-mean)/sd)**2)  #1/(math.sqrt(2*math.pi)*sd)
    return gauss

  def _reward(self):
    current_base_position = self.minitaur.GetBasePosition()
    base_vel = self.minitaur.GetBaseVelocity()
    # forward_reward = current_base_position[0] - self._last_base_position[0]
    forward_reward = self.gauss(base_vel[0], 0.75, 0.25)
    ...
    # print(forward_reward, self._distance_weight * forward_reward, energy_reward, self._energy_weight * energy_reward)
