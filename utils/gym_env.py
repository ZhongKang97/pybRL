...
class EnvSpec(object):
    ...
    def __str__(self):
        res = ''
        res = res + 'Observation Dimension: '+str(self.observation_dim)+'\n'
        res = res + 'Action Dimension: '+str(self.action_dim)+'\n'
        res = res + 'Horizon: '+str(self.horizon)+'\n'
        res = res + 'Num Agents: '+str(self.num_agents)+'\n'
        return(res)
class GymEnv(object):
    ...
    def step(self, action):
        #Clipping actions for minitaur envs, need to unclip later, even Half-Cheetah
        return self.env.step(np.clip(action, -1, 1))
